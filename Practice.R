...
?matplot
matplot(t(FieldGoals/Games),type="p",pch=15:18,col=c(1:4,6))
...
Games
Games[c("KobeBryant","JoeJohnson","LeBronJames"),c("2009","2010","2011","2012","2013")]
Games[1:3,6:10]
...
#Windows
setwd("D:\\Courses\\Udemy R")

#MAC 
#setwd("/Users/"  "/Desktp/R Programming)
...
df
nrow(df) #Tells rows
ncol(df) #Tells columns
head(df,n=10) #gives top 6 rows
tail(df,n=10) #gives last 6 rows
str(df) #structure
...
# $ sign
df
r<-df$Income.Group#== df[,"Internet.users"]
levels(r)
...
mergeddf <- merge(df,df2,by.x="Country.Code",
                  by.y = "Code")
mergeddf
...
qplot(data=mergeddf,x=Internet.users,
      y=Birth.rate,color=Region,size=I(5)
      , shape=I(17))
...
df1 <- data.frame(Code= Country_Code,LET1960 = Life_Expectancy_At_Birth_1960,LET2013=Life_Expectancy_At_Birth_2013)
df1

finaldf <- merge(df,df1,by.x="Country.Code",by.y="Code")
finaldf

qplot(data=finaldf,x=LET1960,y=Fertility.Rate,color=Region,size=I(5))
qplot(data=finaldf,x=LET2013,y=Fertility.Rate,color=Region,size=I(5))
...
f<-ggplot(data=movies,aes(x=Genre,
                          y=CriticRating,
                          color=Genre))
...
s<- ggplot(data=movies,aes(x=CriticRating,y=AudienceRating,color=Genre,size=BudgetMillions))
s+geom_point()
...
w<- ggplot(data=movies,aes(x=CriticRating,y=AudienceRating,color=Genre))
w+geom_point(size=3) + geom_smooth()+ facet_grid(Genre~Year)+coord_cartesian(ylim=c(0,100))
...
#Label Format
h+xlab("Money Access")+ylab("Number of Movies")+theme(axis.title.x = element_text(color="DarkGreen",
                     size =30),axis.title.y = element_text(color="Red",
                                                      size =30))

#TickMark Format
h+xlab("Money Access")+ylab("Number of Movies")+theme(axis.title.x = element_text(color="DarkGreen",
                        size =30),axis.title.y = element_text(color="Red",size =30),
...
#Title
h+xlab("Money Axis")+ylab("Number of Movies")+ggtitle("Movies Money Plot")+theme(axis.title.x = element_text(color="DarkGreen",
size =30),axis.title.y = element_text(color="Red",size =30),
axis.text.x=element_text(size =20),axis.text.y=element_text(size =20),
legend.title =element_text(size =30),legend.text=element_text(size =20),
legend.position=c(1,1),legend.justification = c(1,1),
plot.title=element_text(size=25,color="Darkblue",family = "Courier"))


newmovies<- read.csv(file.choose())
newmovies
